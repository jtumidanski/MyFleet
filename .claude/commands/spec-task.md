...
> You're already inside a task worktree. `/spec-task` must be run from the main repo (`/Users/tumidanski/source/MyFleet`). `cd` there and re-run.
...
From this point on, every file write, read, and shell command MUST use absolute paths under the worktree (e.g., `/Users/tumidanski/source/MyFleet/.worktrees/task-NNN-<slug>/docs/tasks/task-NNN-<slug>/prd.md`). Do NOT write any files under the main repo's `docs/tasks/`.
