...
- [ ] **`provider.go` declares a `Provider` interface** with a `db`-backed implementation and a `NewProvider(db)` constructor, and translates `gorm.ErrRecordNotFound` to a domain `ErrNotFound` sentinel at the boundary (`DOM-10`)
