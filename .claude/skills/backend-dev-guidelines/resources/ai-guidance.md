...
- [ ] **`ToEntity()` method** exists on the Model type in `entity.go` — not just `Make`, which returns `Model` with no error and is not uniform: 16 of 17 domains are `func Make(e Entity) Model`; the exception is `maintenancerecord`, whose model needs its child rows too — `func Make(e Entity, docs []DocumentEntity) Model` (`apps/fleet-service/internal/maintenancerecord/entity.go:44`) (`DOM-02`/`DOM-03`)
...
- [ ] **`provider.go` declares a `Provider` interface** with a `db`-backed implementation and a `NewProvider(db)` constructor — universal, 19 of 19. **Any method that fetches a single record** additionally translates `gorm.ErrRecordNotFound` to a domain `ErrNotFound` sentinel at the boundary — 16 of 19 `provider.go` files, all of them with a single-record fetch method. The remaining 3 have nothing to translate because they expose only list or existence queries, which never surface `gorm.ErrRecordNotFound`: `mileage/provider.go:12-16` (`ListByVehicle` only), `notification/provider.go:29-31` (`ListByUser` only), `platformadmin/provider.go:6-19` (`IsAdmin`/`IsRevoked` only) (`DOM-10`)
