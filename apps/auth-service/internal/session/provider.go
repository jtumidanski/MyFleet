...
// Provider is the read-only interface for refresh-token data access.
type Provider interface {
	FindByHash(hash string) (Model, error)
}
...
func (s *dbProvider) FindByHash(hash string) (Model, error) {
	return database.Query(func() (Model, error) {
		var e Entity
		if err := s.db.Where("token_hash = ?", hash).First(&e).Error; err != nil {
