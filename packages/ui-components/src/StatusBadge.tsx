...
// in the web app's Tailwind content globs (an `@source` in that same file), so
...
export function StatusBadge({ status }: { status: VehicleStatus }) {
  return (
    <span className={`inline-flex rounded-sm px-2 py-0.5 text-xs font-medium ${VARIANT[status]}`}>
      {status}
    </span>
  );
}
